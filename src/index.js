...
addEventListener('load', () => {
  ...
  newProject.newTask(
    ...
    '2022-10-12',
    'High',
    'Finish my To-Do-List application'
  );
  newProject.newTask(
    'Coding Project #2',
    '2022-11-14',
...
newTaskButton.addEventListener('click', () => {
  if (projects.every((projectItem) => projectItem.selected === false)) {
...
submitButton.addEventListener('click', (e) => {
  ...
  if (!formTitle.value || !formDate.value || !formPriority.value) {
    alert(
      'Please enter at a minimum the task title, due date and task priority'
    );
    return;
  }
...
function createTask(task, selectedProject) {
  ...
  bin.addEventListener('click', () => {
    ...
    let taskRemove = selectedProject.tasks.findIndex(
